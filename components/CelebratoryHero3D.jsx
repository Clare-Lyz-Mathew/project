...
const CelebratoryHero3D = () => {
  const mountRef = useRef(null);
  const [activeQuotes, setActiveQuotes] = useState([]);
  const rotationSpeedRef = useRef(5e-3);
  ...
  useEffect(() => {
    ...
    const keyLight = new THREE.DirectionalLight(16777215, 1.5);
    keyLight.position.set(5, 10, 5);
    scene.add(keyLight);
    const rimLight = new THREE.DirectionalLight(8224387, 1);
    rimLight.position.set(-5, 5, -5);
    scene.add(rimLight);
    const ambientLight = new THREE.AmbientLight(4210752, 0.5);
    scene.add(ambientLight);
    const cakeMaterial = new THREE.MeshPhongMaterial({ color: 16777215, shininess: 30 });
    ...
    const animate = () => {
      ...
      const targetSpeed = isHoveringRef.current ? 1e-3 : 5e-3;
