...
const ServiceCategoryPage = ({ service, onBack, onBook }) => {
  ...
  const getThemeStyles = () => {
    switch (service.themeColor) {
      ...
      case "dignified":
        return "bg-charcoal text-white border-stone";
      default:
        return "bg-charcoal text-white";
    }
  };
  return <div className={`min-h-screen ${getThemeStyles()} animate-fade-in relative`}>
      {
  ...
  />
    ...
    /* Hero Content */
      ...
      <section className="container mx-auto px-6 py-32 flex flex-col items-center text-center relative z-10">
        <div className="w-px h-32 bg-silver/20 mb-12" />
        ...
        <div className="flex flex-wrap justify-center gap-12">
          {service.features.map((f, i) => <div key={i} className="flex flex-col items-center gap-3">
              <span className="text-[10px] tracking-[0.5em] uppercase font-bold text-silver/60">{f}</span>
              <div className="w-16 h-px bg-silver/20" />
            </div>)}
        </div>
      </section>
    ...
    /* Related Work Record Section */
      ...
      <section className="py-24 container mx-auto px-6 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end mb-20 border-b border-silver/10 pb-12 gap-10">
          ...
          {service.subFilters && <div className="flex flex-wrap gap-10">
              {service.subFilters.map((filter) => <button
    key={filter}
    onClick={() => setSubFilter(filter)}
    className={`text-[10px] tracking-[0.4em] font-bold uppercase transition-all ${subFilter === filter ? "opacity-100 scale-110 border-b border-current" : "opacity-30 hover:opacity-60"}`}
  >
                  {filter}
                </button>)}
            </div>}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
          {filteredGallery.map((item, i) => <div key={item.id} className={`group relative overflow-hidden h-[700px] shadow-2xl ${i % 2 !== 0 ? "md:mt-32" : ""}`}>
              ...
              <div className="absolute bottom-16 left-16 text-white max-w-md">
                ...
                <div className="w-12 h-px bg-white/40 transition-all duration-500 group-hover:w-full" />
              </div>
            </div>)}
          {filteredGallery.length === 0 && <div className="col-span-full py-40 text-center opacity-40 italic font-serif text-3xl">Archive records coming soon.</div>}
        </div>
      </section>
  ...
  />
    ...
    /* Add-On Marketplace (Conditional View) */
      ...
      {service.id !== "Marketplace" && <AddOnMarketplace
  ...
  >
    ...
    /* Floating Vision Board */
      ...
      {selectedAddOns.length > 0 && <VisionBoard
    eventType={service.id}
    selectedAddOns={selectedAddOns}
