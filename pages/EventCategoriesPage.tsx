...
type EventCategory = 'Weddings' | 'Anniversaries' | 'Birthdays' | 'Reunions' | 'Baptisms' | 'Custom Celebrations' | 'Memorial Services';
type Theme = 'All' | 'International' | 'Minimalist' | 'Cultural' | 'Traditional' | 'Modern';
type Culture = 'All' | 'Hindu' | 'Christian' | 'Muslim' | 'Jewish' | 'Buddhist' | 'Regional' | 'Secular';

const EventCategoriesPage: React.FC = () => {
  const navigate = useNavigate();
  const [selectedCategory, setSelectedCategory] = useState<EventCategory | null>(null);
  const [selectedTheme, setSelectedTheme] = useState<Theme>('All');
  const [selectedCulture, setSelectedCulture] = useState<Culture>('All');
  ...
  const themes: Theme[] = ['All', 'International', 'Minimalist', 'Cultural', 'Traditional', 'Modern'];
  const cultures: Culture[] = ['All', 'Hindu', 'Christian', 'Muslim', 'Jewish', 'Buddhist', 'Regional', 'Secular'];

  const categoryDescriptions: Record<EventCategory, { description: string; packages: string[]; image: string }> = {
    'Weddings': {
    ...
    <div className="min-h-screen bg-seashell">
      ...
      <main className="pt-24">
        ...
        <section className="bg-space-indigo text-white py-16">
          <div className="container mx-auto px-6">
            ...
            <h1 className="text-4xl md:text-6xl font-serif mb-4">Event Categories</h1>
        ...
        {/* Filter Panel */}
        <section className="bg-lilac-ash/20 py-8 border-b border-almond-silk/20">
          <div className="container mx-auto px-6">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              <button
                onClick={() => setShowFilters(!showFilters)}
                className="flex items-center gap-2 px-6 py-3 bg-lilac-ash text-space-indigo rounded-refined hover:bg-almond-silk transition-colors"
              >
              ...
              {showFilters && (
                <div className="flex flex-wrap gap-4 animate-fade-in">
                  <div>
                    <label className="block text-xs text-space-indigo font-bold uppercase mb-2">Theme</label>
                    <select
                      value={selectedTheme}
                      onChange={(e) => setSelectedTheme(e.target.value as Theme)}
                      className="px-4 py-2 bg-seashell border border-almond-silk/20 rounded-refined text-space-indigo"
                    >
                      {themes.map(theme => (
                  ...
                  <div>
                    <label className="block text-xs text-space-indigo font-bold uppercase mb-2">Culture/Religion</label>
                    <select
                      value={selectedCulture}
                      onChange={(e) => setSelectedCulture(e.target.value as Culture)}
                      className="px-4 py-2 bg-seashell border border-almond-silk/20 rounded-refined text-space-indigo"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {categories.map((category) => (
                <div
                  ...
                  className="group relative h-[400px] overflow-hidden bg-dusty-grape rounded-soft shadow-soft hover:shadow-soft-lg transition-all duration-300 cursor-pointer hover:-translate-y-2"
                >
                  <div
                    className="absolute inset-0 bg-cover bg-center transition-transform duration-500 group-hover:scale-110"
                    style={{ backgroundImage: `url(${categoryDescriptions[category].image})` }}
                  >
                    <div className="absolute inset-0 bg-gradient-to-t from-space-indigo via-space-indigo/60 to-transparent" />
                  </div>
                  <div className="absolute inset-0 p-8 flex flex-col justify-end text-white">
                    <h3 className="text-3xl font-serif mb-3">{category}</h3>
                    <p className="text-seashell/90 text-sm mb-4 line-clamp-2">
                      {categoryDescriptions[category].description}
                    </p>
                    <button className="text-almond-silk font-semibold text-sm hover:text-white transition-colors">
                      View Details →
                    </button>
                  </div>
                </div>
      ...
      {selectedCategory && (
        <div className="fixed inset-0 z-50 bg-space-indigo/95 backdrop-blur-xl flex items-center justify-center p-6 animate-fade-in">
          <div className="bg-seashell rounded-soft max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-soft-lg">
            <div className="bg-dusty-grape text-white p-6 rounded-t-soft flex justify-between items-center">
              <h2 className="text-3xl font-serif">{selectedCategory}</h2>
              <button
                onClick={() => setSelectedCategory(null)}
                className="text-white hover:text-almond-silk transition-colors"
              >
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="p-8">
              <div className="mb-8">
                ...
                />
                <p className="text-dusty-grape text-lg leading-relaxed">
                  {categoryDescriptions[selectedCategory].description}
                </p>
              </div>

              <div className="mb-8">
                <h3 className="text-2xl font-serif text-space-indigo mb-4">Available Packages</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {categoryDescriptions[selectedCategory].packages.map((pkg, index) => (
                    <div
                      key={index}
                      className="p-4 border border-almond-silk/20 rounded-refined hover:border-almond-silk transition-colors"
                    >
                      <p className="font-semibold text-space-indigo">{pkg}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 py-4 bg-almond-silk text-space-indigo font-bold uppercase rounded-refined hover:bg-seashell transition-colors"
                >
                ...
                <button
                  ...
                  className="px-8 py-4 border-2 border-almond-silk text-space-indigo font-bold uppercase rounded-refined hover:bg-almond-silk transition-colors"
                >
                  View Add-Ons
                </button>
              </div>
