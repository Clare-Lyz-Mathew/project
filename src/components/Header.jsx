  ...
  >
    ...
    /* Desktop Nav */
        ...
        <nav className="hidden md:flex items-center space-x-10 text-xs font-bold tracking-[0.2em] text-navy/70">
          ...
          {user ? (
            <button onClick={() => handleNavigation("/dashboard")} className={`flex items-center space-x-1 hover:text-navy transition-all ${location.pathname === "/dashboard" ? "text-navy border-b-[1.5px] border-navy pb-1" : ""}`}>
    ...
    /* Mobile Menu */
      ...
      <div className={`md:hidden fixed inset-0 bg-navy transition-transform duration-500 z-50 pt-24 px-6 ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
        ...
        <div className="flex flex-col space-y-8 text-xl font-serif text-mist/90">
          ...
          {user ? (
            <button onClick={() => handleNavigation("/dashboard")} className="hover:text-white transition-colors text-left">Account Dashboard</button>
          ) : (
            <button onClick={() => handleNavigation("/login")} className="hover:text-white transition-colors text-left">Login</button>
          )}

          <button onClick={() => handleNavigation("/booking")} className="text-white border-b border-mauve pb-2 inline-block text-left">Book Now</button>
        </div>
      </div>
    </header>;
};
