import { ADD_ONS, EVENT_ICONS } from "../utils/constants";
import { ArrowRight, X } from "lucide-react";
const VisionBoard = ({ eventType, selectedAddOns, onRemoveAddOn, onProceed }) => {
  const activeAddOns = ADD_ONS.filter((a) => selectedAddOns.includes(a.id));
  return <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[90%] max-w-4xl z-50 animate-slide-up">
      <div className="bg-charcoal/95 backdrop-blur-xl border border-silver/20 shadow-2xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div className="flex-1 flex flex-wrap gap-2 px-8 overflow-hidden max-h-16 md:max-h-none justify-center">
          {activeAddOns.map((addon) => <div key={addon.id} className="bg-slate/50 px-3 py-1 flex items-center gap-2 border border-silver/10">
          ...
          {activeAddOns.length === 0 && <p className="text-xs italic text-silver">Select modular add-ons to build your vision board...</p>}
        </div>
