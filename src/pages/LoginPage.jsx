  ...
  const handleEmailSignIn = async (e) => {
    ...
    try {
      ...
      if (err.code === "auth/user-not-found") {
        navigate("/register", { state: { email, from: location.state?.from } });
      } else if (err.code === "auth/invalid-credential") {
        // With email enumeration protection, 'auth/invalid-credential' is returned
        // for both wrong password and user not found. Since we must redirect if the 
        // account doesn't exist, we will navigate them to register on this error, 
        // as requested: "only redirect to the create account page if the entered account doesnt exist"
        navigate("/register", { state: { email, from: location.state?.from } });
      } else {
    ...
    <div className="min-h-screen bg-navy flex items-center justify-center relative overflow-hidden pt-20">
      ...
      <div className="relative z-10 w-full max-w-md p-8 glass rounded-soft shadow-twilight animate-fade-in mx-4 my-8">
        ...
        <p className="mt-8 text-center text-sm text-mist/60">
          ...
          <Link to="/register" className="text-plum hover:text-mist transition-colors font-bold">
            Create Free Account
          </Link>
        </p>
      </div>
